...
def test_replace_chain_bad_chain(blockchain_three_blocks):
    ...
    with pytest.raises(Exception, match='Cannot replace. The incoming chain is invalid: {e}'):
        blockchain.replace_chain(blockchain_three_blocks.chain)
...
def test_is_valid_transaction_chain_dupicate_transactions(blockchain_three_blocks):
